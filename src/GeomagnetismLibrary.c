 ...
 */
{
    ...
    while(fgets(line, MAXLINELENGTH, stream) != NULL)
    {
                ...
                if(!strncmp(line, paramkeys[i], paramkeylength))
                {
                    ...
                    if(!strcmp(paramkeys[i], paramkeys[INTSTATICDEG]) || !strcmp(paramkeys[i], paramkeys[EXTSTATICDEG]))
                    {
                        ...
                        if(tempint > 0 && allocationflag == 0)
                        {
                            ...
                            (*magneticmodels) = MAG_AllocateModelMemory(numterms);
        ...
        } else if(sscanf(line, "%c,%d,%d", &coefftype, &n, &m) == 3)
        {
            ...
            {
                index = (n * (n + 1) / 2 + m);
                (*magneticmodels)->Main_Field_Coeff_G[index] = gnm;
                (*magneticmodels)->Secular_Var_Coeff_G[index] = dgnm;
                (*magneticmodels)->Main_Field_Coeff_H[index] = hnm;
                (*magneticmodels)->Secular_Var_Coeff_H[index] = dhnm;
            }
        }
    }
    if(header_index > -1)
